...
function gameOverAnimation() {
    ...
    if (currentCell[0] < 9) currentCell[0] += 1;
    else {
        currentCell[0] = 0;
        currentCell[1] += 1;
    }
    if (currentCell[1] < 19) setTimeout(() => gameOverAnimation(), 50);
        ...
        for (let row = 0; row < 4; row++) {
            for (let column = 0; column < 4; column += 1) {
                if (fieldCopy[current.position[1] + row]) {
                    if (
                        fieldCopy[current.position[1] + row][
                            current.position[0] + column
                        ] &&
                        current.shape[row][column]
                    ) {
                        fieldCopy[current.position[1] + row][
                            current.position[0] + column
                        ] = current.color;
